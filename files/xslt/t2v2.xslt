...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="student">
    ...
    <tr>
      ...
      <td><xsl:value-of select="concat($dateDebut, ' ', format-number($heureDebut, '00'), ':', format-number($minuteDebut, '00'))"/></td>
      <td><xsl:value-of select="concat($dateDebut, ' ', format-number($heureFin, '00'), ':', format-number($minuteFin, '00'))"/></td>
    </tr>
  </xsl:template>
</xsl:stylesheet>
